...
class Agent(Agent):
    ...
    def move(self):
        day = math.floor(day_step/8)
        if random.uniform(0, 1) < average_uk[day] / 100:
            if (day_step % 8) - 2 == 0:
...
ax.plot(np.arange(0, steps), new_out, color='blue', label='Infection=1.0')
ax.plot(np.arange(0, steps1), new_out1, color='red', label='Infection=0.8')
ax.plot(np.arange(0, steps2), new_out2, color='green', label='Infection=0.6')
ax.plot(np.arange(0, steps3), new_out3, color='orange', label='Infection=0.4')
ax.plot(np.arange(0, steps4), new_out4, color='pink', label='Infection=0.2')
ax.set_xlabel('Steps')
